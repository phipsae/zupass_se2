...
import { ZKEdDSAFrogPCD, ZKEdDSAFrogPCDArgs, ZKEdDSAFrogPCDPackage } from "@pcd/zk-eddsa-frog-pcd";
import type { NextPage } from "next";

const zupassUrl = "https://zupass.org";

const Frog: NextPage = () => {
  const externalNullifier = generateSnarkMessageHash("consumer-client-nullifier").toString();
  ...
  const { pcd } = useZKEdDSAFrogProof(pcdStr, onVerified, externalNullifier, watermark);
  ...
  return (
    <>
      ...
      <p>
        This page shows a working example of an integration with Zupass which requests and verifies that the user has an
        EdDSA-signed frog, without revealing the user's semaphore identity.
      </p>
      {/* <ExampleContainer> */}
      <button
        onClick={(): void => {
          openZKEdDSAFrogPopup(zupassUrl, window.location.origin + "#/popup", externalNullifier, watermark);
        }}
        disabled={valid}
      >
...
export function openZKEdDSAFrogPopup(
  ...
  const proofUrl = constructZupassPcdGetRequestUrl<typeof ZKEdDSAFrogPCDPackage>(
    urlToZupassWebsite,
    popupUrl,
    ZKEdDSAFrogPCDPackage.name,
    args,
    {
...
function useZKEdDSAFrogProof(
  ...
  externalNullifier: string,
  watermark: string,
): { pcd: ZKEdDSAFrogPCD | undefined; error: Error | undefined } {
  const [error, _setError] = useState<Error | undefined>();
  const zkEdDSAFrogPCD = useSerializedPCD(ZKEdDSAFrogPCDPackage, pcdStr);

  useEffect(() => {
    if (zkEdDSAFrogPCD) {
      verifyProof(zkEdDSAFrogPCD, externalNullifier, watermark).then(onVerified);
    }
...
async function verifyProof(pcd: ZKEdDSAFrogPCD, externalNullifier: string, watermark: string): Promise<boolean> {
  ...
  const sameExternalNullifier =
    pcd.claim.externalNullifier === externalNullifier || (!pcd.claim.externalNullifier && !externalNullifier);
